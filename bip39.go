...
func NewSeedWithErrorChecking(mnemonic, password string) ([]byte, error) {
	...
	mnemonic = CanonicalizeMnemonic(mnemonic)
	ok, _ := IsMnemonicValidPreferEnglish(mnemonic)
	if !ok {
		return nil, ErrInvalidMnemonic
	}
	return NewSeed(mnemonic, password), nil
...
func NewSeed(mnemonic string, password string) []byte {
	...
	normPass := norm.NFKD.String(password)
	return pbkdf2.Key([]byte(normMnemonic), []byte("mnemonic"+normPass), 2048, 64, sha512.New)
}
...
// --- Multi-language support (thread-safe APIs) ---
...
// normalizeMnemonic applies NFKD and collapses whitespace (incl. full-width U+3000) to ASCII spaces.
func normalizeMnemonic(s string) string {
	...
	trimmed := strings.Join(strings.Fields(b.String()), " ")
	return norm.NFKD.String(trimmed)
}

func splitWordsNormalized(s string) []string { return strings.Fields(s) }

// Registry of word lists (immutable).
var registry = map[Language][]string{
...
var registryMap = func() map[Language]map[string]int {
	out := make(map[Language]map[string]int, len(registry))
	for lang, wl := range registry {
		m := make(map[string]int, len(wl))
		for i, w := range wl { m[w] = i }
		out[lang] = m
	}
	return out
}()

// DetectLanguage returns the language whose word list contains all words; if several, tries checksum.
func DetectLanguage(mnemonic string) (Language, error) {
	mn := normalizeMnemonic(mnemonic)
	words := splitWordsNormalized(mn)
	if len(words) == 0 { return "", ErrInvalidMnemonic }

	candidates := make([]Language, 0, len(registry))
	for lang, m := range registryMap {
		ok := true
		for _, w := range words {
			if _, present := m[w]; !present { ok = false; break }
		}
		if ok { candidates = append(candidates, lang) }
	}
	if len(candidates) == 0 {
	...
	for _, lang := range candidates {
		if _, err := entropyFromMnemonicWithWordlist(mn, registry[lang]); err == nil {
			return lang, nil
		}
	}
	return "", ErrInvalidMnemonic
...
// IsMnemonicValidAny validates against all supported languages.
func IsMnemonicValidAny(mnemonic string) (bool, Language) {
	if lang, err := DetectLanguage(mnemonic); err == nil {
		return true, lang
	}
	return false, ""
}

// EntropyFromMnemonicLang validates/decodes with a specific language.
func EntropyFromMnemonicLang(mnemonic string, lang Language) ([]byte, error) {
	return entropyFromMnemonicWithWordlist(normalizeMnemonic(mnemonic), registry[lang])
}

// EntropyFromMnemonicAuto detects language then decodes.
func EntropyFromMnemonicAuto(mnemonic string) ([]byte, Language, error) {
	lang, err := DetectLanguage(mnemonic)
	if err != nil { return nil, "", err }
	ent, err := entropyFromMnemonicWithWordlist(normalizeMnemonic(mnemonic), registry[lang])
	return ent, lang, err
}

// Prefer-English helpers: fast path English, fallback to auto.
func IsMnemonicValidPreferEnglish(mnemonic string) (bool, Language) {
	if _, err := EntropyFromMnemonicLang(mnemonic, English); err == nil {
		return true, English
	}
	if _, lang, err := EntropyFromMnemonicAuto(mnemonic); err == nil {
		return true, lang
	}
	return false, ""
}

func EntropyFromMnemonicPreferEnglish(mnemonic string) ([]byte, Language, error) {
	if ent, err := EntropyFromMnemonicLang(mnemonic, English); err == nil {
		return ent, English, nil
	}
	return EntropyFromMnemonicAuto(mnemonic)
}

// Internal helpers that operate on an explicit word list.

func newMnemonicWithWordlist(entropy []byte, wl []string) (string, error) {
	entropyBitLength := len(entropy) * 8
	checksumBitLength := entropyBitLength / 32
	sentenceLength := (entropyBitLength + checksumBitLength) / 11
	...
	for i := sentenceLength - 1; i >= 0; i-- {
		...
		wordBytes := padByteSlice(word.Bytes(), 2)
		idx := binary.BigEndian.Uint16(wordBytes)
		words[i] = wl[idx]
	}
	return strings.Join(words, " "), nil
}

func entropyFromMnemonicWithWordlist(mnemonic string, wl []string) ([]byte, error) {
	mn := normalizeMnemonic(mnemonic)
	words, ok := splitMnemonicWords(mn)
	if !ok {
		return nil, ErrInvalidMnemonic
	}
	// local word map
	...
	for _, v := range words {
		idx, present := m[v]
		if !present { return nil, ErrInvalidMnemonic }
		checksummedEntropy.Mul(checksummedEntropy, modulo)
		checksummedEntropy.Add(checksummedEntropy, big.NewInt(int64(idx)))
	}
...
// CanonicalizeMnemonic returns a canonicalized mnemonic string suitable for seed derivation
// and validation across toolchains. It applies NFKD and collapses all Unicode whitespace
// (including full-width U+3000) into single ASCII spaces.
func CanonicalizeMnemonic(s string) string {
	return normalizeMnemonic(s)
}
